...
source "hcloud" "microos-x86-snapshot" {
  ...
  location    = "ash"
  server_type = "ccx13" # Using CCX13 for snapshot creation (80GB NVMe, same as control planes)
  snapshot_labels = {
...
# NOTE: We don't use ARM servers in our cluster (only CCX13/CCX33 in ash)
# This snapshot is created only to satisfy kube-hetzner module requirements
# Using fsn1 (Germany) where ARM servers are available for snapshot creation
source "hcloud" "microos-arm-snapshot" {
  image       = "ubuntu-24.04"
  rescue      = "linux64"
  location    = "fsn1"  # Germany - where ARM servers ARE available
  server_type = "cax11" # Smallest ARM server for minimal cost
  snapshot_labels = {
    microos-snapshot = "yes"
    creator          = "kube-hetzner"
  }
